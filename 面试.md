...
3. 实现Callable接口，FutureTask返回值
   **以上线程**是一次性消耗品，执行run()方法后，线程结束后销毁。不能再次start，只能重新建立新的线程对象
...
- 共享数据，多个线程共同操作数据，即为共享数据
- 用同步方法/同步代码块将操作共享数据代码包起来，不能包多了，也不能包少了。
...
ThreadPoolExecutor 不要用 Executors创建，容易OOM（阿里java开发手册）
...
- 改善程序结构，冗长又复杂线程改多个线程，各自独立运行，方便理解修改

## 线程池的优点？

提前创造好多个线程，放到线程池中，用时获取，用完放回，避免浪费，线程池最常用
...
Executors类下方法，定制好定时线程池。ScheduledExecutorService 方法
...
| 任务描述（接口） | 执行器            | 任务       |
| ---------------- | ----------------- | ---------- |
| Future           | TreadPoolExecutor | Runnable   |
...
get（）方法可以当任务结束后返回一个结果，如果工作没结束，则会阻塞线程，直到任务执行完毕
...
ThreadPoolExecutor 类内部类 Worker，有run lock tryLock unlock方法等等
...
2、方法区主要用来存储已被虚拟机加载的类的信息、常量、静态变量和即时编译器编译后的代码等数据。该区域是被线程共享的。
...
引用计数法：某处引用该对象就+1，如果失效-1，当为 0 就会回收，但是 JVM 没有用这种方式，循环引用（A 引用 B,B 引用 A）的情况

引用链法：通过一种 **GC ROOT 的对象**（方法区静态变量引用的对象等、static 变量）来判断，如果有一条链能够到达 GC ROOT 就说明，类似DFS

## 如和判断一个对象是否存活?(或者 GC 对象的判定方法）

以上算法判定一个对象是否能被回收，但满足条件时，不一 **定会被马上回收**。当一个对象不可达 GC Root 时，这个对象并**不会立马被回收**，而是出于一个死缓的阶段，若要被真正的回收需要经历两次标记

如果对象在**可达性**分析中**没有**与 **GC Root 的引用链**，那么此时就会被**第一次标记**并进行一次筛选，筛选的条件是**是否执行 finalize()**方法。当对象没有覆盖 finalize()方法或者已被虚拟机调用过，那么就认为是没必要的。

如果该对象有必要执行 finalize()方法，那么这个对象将会放在一个称为 **F-Queue 的对队列**中，虚拟机会触发一个 Finalize()线程去执行，此线程是**低优先级**的，并且虚拟机不会承诺一直等待它运行完，这是因为如果 finalize()执行**缓慢或死锁**，那么就会造成 **FQueue 一直等待**，造成了内存回收系统的崩溃。GC 对处于 F-Queue 中的对象进行第二次被标记，这时对象等待回收。
...
|  8   |  1   |  1   | 20      |
| :--: | :--: | :--: | ------- |
| Eden | From |  To  | PermGen |

​						youngGC								15 次survive ====>		如今是MetaSpace
...
复制算法将可用内存按容量划分为相等的两部分，每次只使用其中的一块，当一块内存用完时，就将还存活的对象复制到第二块内存上，然后一次性清楚完第一块内存，再将第二块上的对象复制到第一块。内存的代价太高

将内存划分为8:1:1 三部分，较大那份内存交 Eden 区，其余是两块较小的内存区叫 Survior 区。Survior不够，老年区担保
...
虚拟机把 Class 文件加载到内存，并对数据进行**校验**，**解析**和**初始化**，最终形成可以被虚拟机直接使用的 java 类型。
...
1. 内存结构：栈（局部变量）、堆（new 出来的结对象（成员变量）数组等）

2. 变量：成员变量 vs 局部变量 (方法内、方法形参、构造器内、构造形参、代码块内)
...
抽象类表示对象是什么。接口表示的是，这个对象能做什么。比如，男人，女人，这两个类，他们的抽象类是人。说明，他们都是人。
...
## 数据库连接池execute与executeUpdate的相同点

都可以执行增加，删除，修改

statement 包含sql命令，就能远程操作数据库
...
**Collection：**单列集合接口，有List和Set
**Collections：**对集合操作的工具类，操作Collection和Map的工具类，包含排序和二分查找的方法

## int与Integer 的区别?

int是Java语言提供的八种原始数据类型之一，对象的属性默认值为0
...
当往容器里存放整数，无法直接存放int而是将int类型转换为Integer存放。
...
URL是Internet上用来描述信息资源的字符串，主要用在各种客户程序和服务器程序上。采用URL可以用一种统一的格式来描述各种信息资源，包括文件、服务器的地址和目录等。
...
**变量** 不能修改
属性，可以显示初始化/代码块初始化/构造器初始化只赋值一次
局部变量，变量，当修饰形参，此形参是一个常量，一旦赋值不能修改
...
## final修饰基础数据类型和引用数据类型

基础数据类型，final修饰肯定不变		
引用数据类型，final修饰引用地址不变，值可变——String、StringBuffer、ArrayList、HashSet、HashMap等。

## 重载和重写

**方法重写（覆盖）Override：**子继承父	子重写覆盖父亲
**方法重载Overload：**相同方法名，有不同参数列表

Overload的方法是否可以改变返回值的类型？
Overload和Override是java多态性的不同表现，其中，重载是在一个类中多态性的一宗表现，是指在一个类中定义了多个同名的方法，它们或有不同的参数个数或有不同的参数类型。在使用重载时，需要注意:

1 重载通过不同的方法参数来区分
2 不能通过方法的访问权限，返回值类型和抛出的异常来进行重载
3 对于继承类来说，如果基类方法的是private，那派生类不能对其进行重载，如派生类定义一个同名函数，这只是一个新的方法，不会达到重载的作用

重载与覆盖之间的区别：

1. 覆盖是垂直关系。重载是水平关系
2. 覆盖只由一个方法或只由一对方法产生关系；重载多个方法间关系。
3. 覆盖要求参数类表相同，重载要求参数列表不同
4. 覆盖关系中，调用方法体是根据对象的类型（对象对应的存储空间类型）来决定的；而重载关系是根据调用时的实际参数与形参列表来选择方法体的。
5. 覆盖不能改变返回值的类型，重载是可以改变返回值的类型

派生类的覆盖方法所抛出的异常，必须和基类中被覆盖方法所抛出的异常一致或者是他的子类。

## Comparable与 Comparator的区别？

Comparable 是自然排序。让元素自身具备**比较性**。覆盖int compareTo(T o)方法。Comparable很多java.lang的常用类都实现了这个接口，基础数据类型的包装类 
...
Forward是服务器内部的重定向，服务器直接访问目标地址的URL，把那个URL的响应内容读取过来，而客户端并不知道，因此，在客户端浏览器的地址栏里不会显示转向后的地址，还是原来的地址

Redirect则是客户端重新定向，即客户端浏览器会获取跳转后的地址，然后重新发送请求，因此，浏览器中会显示跳转后的地址。同时由于这种方式比forward方式多了一次网络请求，forward效率更高
...
把多个线程访问资源的操作放在一个标记Synchronized的方法中，就能够保证这个方法在同一时间只能被一个线程来访问，从而保证了多线程访问的安全性。然而，当一个方法的方法体规模非常大的时候，把该方法声明为synchronized会大大影响程序的执行效率，所以提供代码块

可把任意代码段声明为synchronized，也可以指定上锁的对象，有非常高的灵活性。
...
1 Iterator 迭代器方式，迭代器：不同集合提供统一的遍历操作接口
...
```java
dataType[] arr = new dataType[size];				//建议这么写
```
...
实际问题，我们要给属性赋值加入额外的限制条件，这个条件不能在属性声明时体现，只能通过方法进行限制条件添加，我们要避免用户用对象给属性赋值。则我们需要将属性声明为私有的(private)，体现了封装性
...
八种基本类型：**[int short long]**  **[double float]** char bool byte
...
ArrayList的SubList方法返回值不是ArrayList，是内部类SubList。
...
是一个类型修饰符。volatile修饰的变量有**可见性**，即保证线程读取到的是最新更新的值。线程不拷贝内存变量而是直接读取内存中的变量，当内存中变量被其他线程修改后线程能立马知道。同步是锁定变量，只允许一个线程对其进行操作，是**原子性**动作。volatile修饰不一定是线程安全的。

某共享变量，每个线程都缓存一个该变量的副本。当一个线程更新其副本(高速缓存区)时，其他的操作单元可能没有及时发现，进而产生缓存一致性问题。

在JMM内存模型，堆内存上面叠放线程内存，每次都打到底，改了也会去更新别的线程内存的副本。
...
jdk 8 注解新特性，注解就是类中的特殊标记，反射中读标记，在编译，类加载，运行时读取
...
@TableField(exist = false) //忽略该属性和字段映射
...
@EnableEurekaServer		//打开eureka服务器
@EnableEurekaClient		//打开eureka客户端
@MapperScan			//扫描哪个地址包
@EnableFeignClients		//启用feign客户端
@ServletComponentScan	//此注解让拦截器生效
@EnableZuulProxy	//打开Zuul网关代理
```
...
@ComponentScan("util") //跨模块的注解，适用工具类
...
五大组件：不包括Feign FeignClient 动态代理，地址调用到服务

Eureka
Ribbon
config
Hystrix 熔断是什么 服务降级 线程隔离
Zuul 微服务网关 内部几个filter 前置 后置 环绕自己配

![image-20200515010722211](./面试img/image-20200515010722211.png)

![image-20200515011635380](./面试img/image-20200515011635380.png)

![image-20200515011744068](./面试img/image-20200515011744068.png)
